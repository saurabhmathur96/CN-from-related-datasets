...
def estimate_parameters(
        cnet: CutsetNetworkNode, D: Dataset, s: float = 1, leaf_only=False):
    parameters = []

    if leaf_only:
        iterator = ((node, Di) for node, Di in depth_first_order(
            cnet, D) if isinstance(node, LeafNode))
    else:
        ...
        if isinstance(node, LeafNode):
            if len(Di) == 0:
                ...
                cpds = est.get_parameters(
                    prior_type="dirichlet", pseudo_counts=s)
                parameters.append(cpds)
        else:
...
def set_parameters(cnet: CutsetNetworkNode, parameters: List, leaf_only=False):
    ...
    if leaf_only:
        leaves = (node for node in depth_first_order(
            cnet) if isinstance(node, LeafNode))
